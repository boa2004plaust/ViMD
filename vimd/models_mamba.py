...
class VisionMamba(nn.Module):
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False,
        ...
        if not self.if_bidirectional:  #
            ...
            for layer in self.layers:
                ...
                # print("i:", i, ",hidden_states:", hidden_states.size(), ",residual:", residual.size())
                hidden_states_features.append(hidden_states)
        ...
        if not self.fused_add_norm:
            ...
            for j in range(0, 24):
                hidden_states_features[j] = fused_add_norm_fn(
                    self.drop_path(hidden_states_features[j]),
                    self.norm_f.weight,
                    self.norm_f.bias,
                    eps=self.norm_f.eps,
                    residual=residual_features[j],
                    prenorm=False,
                    residual_in_fp32=self.residual_in_fp32,
                )
        ...
        if self.if_cls_token:
            if self.use_double_cls_token:
                ...
                if self.use_middle_cls_token:  #
                    ...
                    # print("hidden_states_features:", hidden_states_features[0].size())
                    return hidden_states_features, hidden_states  # return 这个（16,192）
    ...
    def forward(self, x, return_features=True, inference_params=None, if_random_cls_token_position=False,
                if_random_token_rank=False):
        hidden_states_features, x = self.forward_features(x, inference_params,
                                                          if_random_cls_token_position=if_random_cls_token_position,
                                                          if_random_token_rank=if_random_token_rank)
        # final_feature = x  # (16,192)
        x = self.head(x)
        if self.final_pool_type == 'max':
            x = x.max(dim=1)[0]
        return hidden_states_features, x
